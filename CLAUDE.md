...
- **Reorder Requests Table** (auto-created on first push) — master request records
  - Fields: Reference ID, Title, Subtitle, Created Date, Approval Date, Order Sent Date, Shipments Config (JSON), S1/S2 Method/Destination/Description (backward compat), S1–S10 Name (shipment labels), Total Items/Units/Pieces, Total Cost RMB/USD, Exchange Rate, Status (Draft/For Approval/Approved), Notes
  - **Milestone dates** (Created Date / Approval Date / Order Sent Date): three manual date pickers in the editor header, persisted in state as `dateCreated`/`dateApproved`/`dateSent`. Shown stacked on every list card (Created / Approved / Order Sent). "Created" backfills from the request's `createdAt` if never set; it also serves as the document date in the Sir Ohad / Supplier views and Excel exports (the old standalone "Date" field was removed). "Order Sent" = date the order was sent to the supplier (pending payment). Synced to Airtable as date fields and read back via `buildStateFromAirtable`, so they survive the silent Load-from-Airtable overwrite. Schema-ensure gated by `milestoneDatesAdded` flag. Legacy state/records that only have `hdrDate`/`Date` fall back into `dateCreated`. **Note:** the legacy `Date` Airtable column is no longer written by the app and must be deleted manually in the Airtable UI (the API cannot delete fields).
...
## App Structure
- **List Screen**: Shows all saved re-order requests as cards (title, date, item/unit/piece counts). "+ New Request" button to create. Click a card to open it. Delete button per card (hidden for Approved/Shipped requests). Below the request list is an **ASIN Summary** section (collapsible, blue header) aggregating every item across all local requests — columns: ASIN, Description, Qty (Units), Request ID, Status. Supports text filter, status dropdown filter, and sortable columns (default sort by status). Click a row to jump into that request. Below ASIN Summary is an **Archived** section (collapsible, teal header — collapsed by default) that contains all Delivered and Cancelled requests so the main list stays focused on in-flight work. Statuses to archive are defined in `ARCHIVED_STATUSES`; expand/collapse state lives in `ARCHIVED_STATE` and is session-only.
